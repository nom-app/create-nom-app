...
const statsHandler: ICompiler.Handler = (err, stats) => {
  console.log(
    `Compile your app for production with ${chalk.green(packageManager === 'yarn' ? 'yarn build' : 'npm run build')}.`
  )
...
function isProjectRootAString(arg: any): arg is string {
  return typeof arg === 'string'
}

function main(): void {
  if (!isProjectRootAString(projectRoot)) {
    console.error(
